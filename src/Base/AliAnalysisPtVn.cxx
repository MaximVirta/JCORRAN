...
void AliAnalysisPtVn::UserExec(Option_t *option) {
  ...
  for (int i = 0; i < nCorr; i++) {
    if (TMath::Finite(corr[i])) corrProfiles[i]->Fill((Float_t)(centralityBin)+0.5, corr[i],(i>=nCorr-2)?(Qpos[0]*Qneg[0]).Re()*psubMid[0]:1.); //Event weight for v22-pt gap for consistency with pub.
  }
